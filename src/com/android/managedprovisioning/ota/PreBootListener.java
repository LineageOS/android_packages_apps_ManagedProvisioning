...
public class PreBootListener extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (deviceOwnerUserId != UserHandle.USER_NULL) {
            addDeviceOwnerTasks(deviceOwnerUserId, context);
        }

        for (UserInfo userInfo : mUserManager.getUsers()) {
            if (userInfo.isManagedProfile()) {
                addManagedProfileTasks(userInfo.id, context);
            } else {
    ...
    @VisibleForTesting
    PreBootListener(TaskExecutor taskExecutor) {
